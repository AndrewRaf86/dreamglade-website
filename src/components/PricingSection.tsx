...
export default async function PricingSection() {
  const { pen, usd, usdRate } = await getPricing();

  const usdNote = usdRate
    ? `Approx. USD at today's rate (1 PEN = $${usdRate.toFixed(4)})`
    : null;
      ...
      <div className="container">
        <div className="section-head">
          ...
          <p className="lede" style={{ marginTop: 12 }}>
            Priced in Peruvian Soles (S/.) per person per day. All-inclusive: accommodation, meals, ceremonies, plant dieta, yoga, breathwork, sauna, and integration support.
          </p>
        </div>
        ...
        <div className="pricing-grid">
          <article className="pricing-card">
            ...
            <p className="pricing-card__price">
              <span className="pricing-card__price-num">S/. {pen.shared}</span>
              <span className="pricing-card__price-unit">per person / day</span>
            </p>
            {usd.shared && (
              <p style={{ fontSize: 13, color: "var(--muted)", margin: "0 0 4px" }}>
                ≈ ${usd.shared} USD <em style={{ fontStyle: "normal", opacity: 0.75 }}>(approximate)</em>
              </p>
            )}
          ...
          <article className="pricing-card">
            ...
            <p className="pricing-card__price">
              <span className="pricing-card__price-num">S/. {pen.single}</span>
              <span className="pricing-card__price-unit">per person / day</span>
            </p>
            {usd.single && (
              <p style={{ fontSize: 13, color: "var(--muted)", margin: "0 0 4px" }}>
                ≈ ${usd.single} USD <em style={{ fontStyle: "normal", opacity: 0.75 }}>(approximate)</em>
              </p>
            )}
            <p style={{ color: "var(--muted)", fontSize: 14, margin: "0 0 20px" }}>
        ...
        <div style={{ marginTop: 32, padding: "16px 20px", background: "var(--cream-warm)", borderLeft: "3px solid var(--gold)", borderRadius: 4, maxWidth: 760 }}>
          <p style={{ fontSize: 13, color: "var(--muted)", margin: 0, lineHeight: 1.6 }}>
            <strong style={{ color: "var(--ink)", fontWeight: 600 }}>Prices are in Peruvian Soles (S/.).</strong>{" "}
            Any USD equivalent shown is calculated at the day&apos;s exchange rate and is approximate — the actual USD amount will depend on when your deposit is paid.{" "}
            {usdNote && <span>{usdNote}. </span>}
